...
# -*- coding: utf-8 -*-
"""
Created on Wed May 14 17:01:43 2025
...
ax2.axhline(y=0.5, color='black', alpha=0.4, linestyle='--', linewidth=1)
ax2.text(-0.04, 1.07, 'c)', transform=ax2.transAxes,
              fontsize=12, fontweight='bold', va='top', ha='right')
"""
# Zoom inset on the experimental data
#axins2 = ax2.inset_axes([0.72, 0.63, 0.24, 0.24])
for i, (linker, col_indices) in enumerate(columns_of_interest.items()):
    if linker in ['LS12', 'LS11']:
        df_selected = df_L0_L1[[df_L0_L1.columns[col_indices['h'][0]],
                                df_L0_L1.columns[col_indices['h'][1]]]]
    ...
    avg_absorbance = (df_h[f'Absorbance_Norm_{linker}_h'].iloc[:min_len] +
                      df_c[f'Absorbance_Norm_{linker}_c'].iloc[:min_len]) / 2
"""
    axins2.plot(temp_range, avg_absorbance, '-', color=colors_1[i], alpha=0.7)
...
ax3.plot(df1["Temperature(C)"], 1 - df1["Fraction of bases unpaired at equilibrium"], color=colors_1[0], label=custom_names["Y0"],linestyle="--")
...
ax3.text(-0.04, 1.07, 'a)', transform=ax3.transAxes, fontsize=12, fontweight='bold', va='top', ha='right')

# Configuración del zoom
axins = ax3.inset_axes([0.7, 0.58, 0.25, 0.25])  # (x0, y0, width, height) del panel secundario
...
ax3.set_ylabel("$P\,(\mathrm{bound \,state})$", fontsize=12)
axins.grid(True)
...
ax3.axhline(y=0.5, color='black', alpha=0.4, linestyle='--', linewidth=1, xmin=0.47, xmax=1)
